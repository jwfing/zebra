...
public class BloomFilter implements Serializable {
    ...
    static final Charset charset = Charset.forName("ISO-8859-1"); // encoding used for storing hash values as strings
    ...
    public BloomFilter(double falsePositiveProbability,
        ...
        this(Math.ceil(-(Math.log(falsePositiveProbability) / Math.log(2)))
                ...
                expectedNumberOfElements, (int) Math.ceil(-(Math
                        .log(falsePositiveProbability) / Math.log(2)))); // k = ceil(-log_2(false
                                                                         // prob.))
    }
...
//        numberOfAddedElements = bbuf.getInt(index + 12);
    ...
    public int getDumpSize() {
        int bitsetBytes = (bitSetSize + 7) / 8;
        return 4 * 4 + bitsetBytes;
    }
    ...
    public String toBinary() {
        int bitsetBytes = (bitSetSize + 7) / 8;
        ByteBuffer bbuf = ByteBuffer.allocate(getDumpSize());
        ...
        int index = bitsetBytes;
        bbuf.putInt(index, this.bitSetSize);
        bbuf.putInt(index + 4, this.expectedNumberOfFilterElements);
        bbuf.putInt(index + 8, this.k);
        bbuf.putInt(index + 12, this.numberOfAddedElements);
        return new String(bytes, charset);
    }

    public boolean add(BloomFilter bf) {
        if (bf.bitSetSize == this.bitSetSize
                && bf.expectedNumberOfFilterElements == this.expectedNumberOfFilterElements
                && bf.k == this.k && bf.getBitSet() != null) {
            bitset.or(bf.getBitSet());
            this.numberOfAddedElements += bf.count();
            return true;
        }
        return false;
    }
    ...
    public void add(long element) {
        add(Long.toString(element).getBytes(charset));
    }

    public void add(int element) {
        add(Integer.toString(element).getBytes(charset));
    }
    ...
    public boolean contains(long element) {
        return contains(Long.toString(element).getBytes(charset));
    }

    public boolean contains(int element) {
        return contains(Integer.toString(element).getBytes(charset));
    }

    /**
     * filter collection<E> with this BloomFilter
     */
    public Collection<? extends Object> filter(Collection<? extends Object> coll) {
    ...
    public double getExpectedBitsPerElement() {
        return this.bitSetSize / (double) expectedNumberOfFilterElements;
    }
