...
public class BDBStorageImpl implements UrlStorage {
    private static final Logger logger = Logger.getLogger(BDBStorageImpl.class.getName());

    private Environment env = null;
    private WorkQueues repeatUrlDB = null;
    private WorkQueues onceUrlDB = null;

    private static String urlDir = Configuration.getStringProperty(Configuration.PATH_URLPOOL_DIR,
            "./urlPool");
    ...
    public void initialize() {
        try {
            ...
            this.repeatUrlDB = new WorkQueues(this.env, "RepeatURLsDB", isResume);
            this.onceUrlDB = new WorkQueues(this.env, "OnceURLsDB", isResume);
            logger.info("initialize urlPool with directory=" + urlDir + ", resume=" + isResume);
        ...
        for (UrlInfo url : urls) {
            ...
            if (null != this.repeatUrlDB) {
                try {
                    ...
                    logger.warn("encounter DatabaseException, cause:" + e.getMessage());
                    return false;
                }
            }
        }
    ...
    public boolean addOnceUrls(List<UrlInfo> urls) {
        UrlSeed seed = null;
        for (UrlInfo url : urls) {
            seed = UrlSeed.generateInstanceFromUrlInfo(url, 0);
            if (null == seed) {
                continue;
            }
            logger.debug("add once url=" + seed.getUrl());
            if (null != this.onceUrlDB) {
                try {
