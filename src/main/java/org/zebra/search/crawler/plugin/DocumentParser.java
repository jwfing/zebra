...
public class DocumentParser implements Processor {
	...
	public boolean process(CrawlDocument doc, Context context) {
		...
		try {
			...
			logger.warn("failed to parse document. url=" + doc.getUrl()
					+ ", cause=" + e.getMessage());
			return false;
		}
	...
	private void parseHeadBase(CrawlDocument doc, NodeList node) {
		...
		if (null != headNodes && headNodes.size() > 0) {
			...
			NodeList baseNodes = headChildrens.extractAllNodesThatMatch(
					orFilter2, true);
			if (null != baseNodes && baseNodes.size() > 0) {
				try {
					for (NodeIterator i = baseNodes.elements(); i
							.hasMoreNodes();) {
						...
						if (null != base && !base.isEmpty()) {
							logger.debug("BASENODE FOUND. docUrl="
									+ doc.getUrlInfo().getUrl() + ", base="
									+ base);
