...
//	private static final String SUFFIX_MINUS = "-";
	...
	private static final String[] FORBIDDEN_TERMS = { "blog", "thread", "bbs",
			"forum", "download", "javascript", "books", "copyright", "video",
			"music", "schedule", "picture", "comment", "price", "about" };
	...
	public boolean process(CrawlDocument doc, Context context) {
		...
		try {
			for (NodeIterator i = links.elements(); i.hasMoreNodes();)
			{
				...
				int parentDepth = 0;
				//get the depth of the doc for followlink exp added by deepFollow 
				if (isDeepFollow() && doc.getUrlInfo().getFeature(ProcessorUtil.COMMON_PROP_DEPTH) != null) {
					parentDepth = Integer.valueOf(
							(String) doc.getUrlInfo().getFeature(ProcessorUtil.COMMON_PROP_DEPTH));
				}
	
				// go depth 2 level at most, and "index" in url is necessary added by deepFollow
				if (parentDepth > 0
	...
	private NodeList extractLinkNodes(NodeList nodeList) {
		NodeFilter[] linkFilters = { new TagNameFilter("A"),
				new TagNameFilter("AREA"), new TagNameFilter("FRAME"),
				new TagNameFilter("IFRAME"), new TagNameFilter("LAYER"),
				new TagNameFilter("ILAYER") };
	...
	private boolean isNewsPage(String url, boolean enableList) {
		...
		String restUrl = url.substring(1 + url.indexOf('/',
				HTTP_PROCOTOL.length()));// skip host part
...
/*	protected String normalizeUrl(String url, String origEncoding) {
		if (url == null) {
			return url;
		}
		url = url.replaceAll("\\s+", "+");
		url = urlEnc(url, origEncoding);
		url = urlSpecialDecode(url);

		// add by fengjw
		if (url.endsWith(PREFIX_5C22)) {
			url = url.substring(0, url.length() - PREFIX_5C22_LEN);
		}
		if (url.endsWith(SUFFIX_MINUS)) {
			url = url.substring(0, url.length() - SUFFIX_MINUS.length());
		}
		if (url.endsWith(SUFFIX_PLUS)) {
			url = url.substring(0, url.length() - SUFFIX_PLUS.length());
		}

		int invadIndex = url.indexOf(HTTP_PROCOTOL);
		if (invadIndex > 0) {
			url = url.substring(invadIndex);
		}
		// end end by fengjw
		invadIndex = url.indexOf("#");
		if (invadIndex > 0) {
			url = url.substring(0, invadIndex);
		}

		if (this.rmSidFromUrl) {
			url = removeSidFromUrl(url);
		}
		return url;
	}

	private String urlSpecialDecode(String url) {
		if ((url == null) || (url.length() == 0)) {
			return url;
		}
		url = url.replaceAll("&amp;", "&");
	...
	private String urlEnc(String u, String origEncoding) {
		StringBuffer r = new StringBuffer();
		try {
			for (int i = 0; i < u.length(); ++i) {
				int n = u.codePointAt(i);
				char c = u.charAt(i);

				if ((n >= 123) || (n <= 32) || (n == 92)
						|| ((n >= 91) && (n <= 94)) || (n == 96) || (n == 34))
					r.append(URLEncoder.encode(String.valueOf(c), origEncoding));
				else
					r.append(c);
			}
		} catch (Exception e) {
