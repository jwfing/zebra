...
public class HttpClientFetcher implements Fetcher {
	...
	public static final int MAX_DOWNLOAD_SIZE = 1024* 1024 * 10;
	...
	static {
		...
		params.setParameter(HTTP_USERAGENT,
				Configuration.getStringProperty(Configuration.PATH_FETCHER_USERAGENT, "zebraCrawler"));
...
/*		ConnManagerParams.setMaxConnectionsPerRoute(params, connPerRouteBean);
		ConnManagerParams.setMaxTotalConnections(params,
				Configuration.getIntProperty(Configuration.PATH_FETCHER_MAXCONN_TOTAL, 100));
*/
		...
		if (Configuration.getBooleanProperty(Configuration.PATH_FETCHER_ENABLEHTTPS, false)) {
			schemeRegistry.register(new Scheme(PROTOCOL_HTTPS, SSLSocketFactory.getSocketFactory(), 443));
		}
...
//				if (size <= 0) {
//					entity.consumeContent();
//					logger.warn("failed to download document. url=" + toFetchURL);
