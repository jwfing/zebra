...
public class DefaultAllocator implements Allocator{
			...
			while(isAlive()) {
				if (selector != null && collection != null) {
					...
					if (null == urls || urls.size() == 0) {
						...
						logger.info("retrieve " + urls.size() + " urls with level="
								+ this.level + " repeat=" + this.isRepeat);
	...
	public void initialize() {
		...
		for (int i = 0; i < scannerThreadnum; i++) {
			level = Configuration.getIntProperty(
					Configuration.PATH_ALLOC_SCAN_THREAD + i + Configuration.CONST_LEVEL_SUFFIX,
					1);
			interval = Configuration.getIntProperty(
					Configuration.PATH_ALLOC_SCAN_THREAD + i + Configuration.CONST_INTERVAL_SUFFIX,
					3);
			if (level < 0 || interval <= 0) {
				logger.warn("invalid configuration for crawler.alloc.scan, item index:"
						+ i + ", level:" + level + ", interval:" + interval);
				continue;
			}
		...
		interval = Configuration.getIntProperty(
				Configuration.PATH_ALLOC_SCAN_ONCE_INTERVAL, 3);
		if (interval <= 0) {
